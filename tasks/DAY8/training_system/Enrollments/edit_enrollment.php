...
<div class="container mt-5 bg-info">
    <div class=" row ">
    <form class="was-validated " method="POST" enctype="multipart/form-data" action="update_enrollment.php?id=<?=$id?>">
        <div class="row justify-content-center d-flex mt-5">
            ...
            <div class="  mt-3  ">
             ...
             <option selected  value="<?= htmlspecialchars($enrollment["id"]) ?>"><?= htmlspecialchars($enrollment["NAME"]) ?></option>
             ...
             <?php while($row=mysqli_fetch_assoc($corse)){
              ...
              <option selected  value="<?= htmlspecialchars($row["id"]) ?>"><?= htmlspecialchars($enrollment["title"]) ?></option>
              <?php }else{ ?>
            ...
            <input type="number" class="form-control" name="grade" value=<?=$enrollment["grade"] ?> required>
            </div>
